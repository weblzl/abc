## 服务器升级，所有访问暂停，等待升级完成恢复04/27
## 最近几天`worker.js  workerm3u.js`后台显示每天访问 `IP` 数量`2000+` ，`Mitv.js`每天访问 `500+`  反馈部署访问不了使用不了的，可能是部署时没能准确部署/也有可能是当地网络访问不了
## 重要提示：因部分不同厂家域名被墙导致不能访问！（不是worker脚本本身问题，目前js都是正常的，请尝试更换不同域名测试）
## 组播更新【更新代码：解决无法获取数据问题  更新时间：0419/16:00】

## 修复mitv BUG `js` 脚本 ，请重新复制`JS`重新部署【0417/11:45更新】
